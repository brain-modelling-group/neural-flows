...
function cmap = compute_circular_jet_hue(u, v)
%   compute_circular_jet_hue colour codes a flow field (u, v) as an image img.
...
%   $Date: 2007-10-31 21:20:30 (Wed, 31 Oct 2006) $
    ...
    colorwheel = make_colour_wheel();
    ncols = size(colorwheel, 1);

    % Norm -- 
    rad = sqrt(u.^2 + v.^2);
    % Angle between flow components
    a = atan2(-v, -u)/pi;
...
function colorwheel = make_colour_wheel()
