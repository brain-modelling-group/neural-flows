...
% $Revision: 5.1 $
% 
% PLUS small correction (in 2017, by JR, on advice of RA) to the window
% times (only affects output t_out, not the actual corrgram calculation)


%error(nargchk(2,5,nargin))
narginchk(2,5) % suggested by matlab to replace above
verbose = 0;
...
%t = (1:nx/size(Xi,2):nx)';
t = (1:(window-noverlap):(1+length(x)-window)).'; % Romesh fix
l = (-maxlag:maxlag)';
