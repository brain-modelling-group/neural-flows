function [flowField, int_dF, errorData, errorReg, poincare_idx, time_flow] =   bst_opticalflow(F, FV, t, idx_start, idx_end, hs_smoothness)
...
%   hs_smoothness     - Smoothness parameter of Horn-Schunck method rakes values in [0, 1]
...
% end
% 
% % Check that the indices are not the same one
% if tStartIndex == tEndIndex,
%    tEndIndex = tEndIndex + 1;end
% end 
% 
% 
...
tEndIndex   = idx_end;
tStartIndex = idx_start;


time_flow  = t(tStartIndex:tEndIndex);      % Interval of flow calculations. 
