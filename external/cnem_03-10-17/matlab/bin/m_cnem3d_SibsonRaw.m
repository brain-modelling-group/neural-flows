...
function data_out=m_cnem3d_SibsonRaw(XYZ_Noeud,IN_Tri_Ini,XYZ_Point)
    ...
    [IN_New_Old,IN_Old_New,INV_NN,PNV_NN,Ind_Point,Nb_V,Ind_V,FF,Grad]=...
    cnem3d(3,XYZ_Noeud',IN_Tri_Ini',XYZ_Point',Type_Call_Tet);
    ...
    FF_sp=sparse(i_Point',Ind_V,FF',size(XYZ_Point,1),nb_noeud_ini+I);
    data_out.FF_sp=FF_sp(Inv_Ind_Point,:);
    
    GX_sp=sparse(i_Point',Ind_V,Grad(1,:)',size(XYZ_Point,1),nb_noeud_ini+I);
    data_out.GX_sp=GX_sp(Inv_Ind_Point,:);
    
    GY_sp=sparse(i_Point',Ind_V,Grad(2,:)',size(XYZ_Point,1),nb_noeud_ini+I);
    data_out.GY_sp=GY_sp(Inv_Ind_Point,:);

    GZ_sp=sparse(i_Point',Ind_V,Grad(3,:)',size(XYZ_Point,1),nb_noeud_ini+I);
    data_out.GZ_sp=GZ_sp(Inv_Ind_Point,:);
    
    Nb_V=double(Nb_V);
    Nb_V(Ind_Point)=Nb_V;
    data_out.Nb_V=Nb_V;

    data_out.Mat_INT_NN=INT_NN;
